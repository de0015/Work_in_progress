...
state_filter_2<- c("Vermont","New York","Pennsylvania","New Jersey","West Virginia","North Carolina","Texas","Puerto Rico","Texas","California","Illinois","District of Columbia", "Florida", "United States")
...
merge_test <- merge(global_test, compar, by = "Country")

merge_test <- merge_test[,c(1,5,6,7,8,4)]
        ...
        mainPanel(
          mainPanel(
            tabsetPanel(
              ...
              tabPanel("Vaccines List", DT::dataTableOutput("mytable6"))
            )
          )
        )
  )
